...
def traverse_voxels(voxel_references, ray_partition, chunks_per_compute, epsilon=1e-6):
    ...
    # Calcul{ate mask, t_enter, and t_exit for max voxels that fit into memory
    masks = {}
    t_enter_coords = {}
    t_exit_coords = {}
    t_entry_radiis = {}
    ...
    distance_to_point = np.linalg.norm(filtered_points - filtered_origins, axis=1)
    yet_to_hit_rays = np.logical_or(distance_to_point > distance_to_exit, np.isnan(filtered_points).any(axis=1)) 
...
def get_voxel_metrics(intersections_files, lambda_1, is_leaf_true=True, debug=True, epsilon=1e-9):
    ...
    def calculate_voxel_metrics_per_voxel(voxel_df, min_rays=6, epsilon=1e-9):
        ...
        viewing_angles = voxel_df['viewing_angle'].values[mask]
        ...
        bin_centres, LIAD_values, angles = calculate_LIAD(leaf_points)
